...
function CashBook() {
  ...
  const generateCashBook = async () => {
    try {
      setIsLoading(true);
      const response = await axiosInstance.get(`/vouchers/branch/${formData.branchId}/cashbook/${formData.date}`);

      if (response.data.success) {
        let totalCredit = response.data.totals.totalCredit;
        let totalDebit = response.data.totals.totalDebit;
        ...
        const openingBalance = response.data.openingBalance || 0;
        const closingBalance = response.data.totals.closingBalance || 0;
                ...
                <tbody>
                  ${transactions
                    ...
                    <tr>
                      ...
                      <td>${Math.round(entry.balance || '0')}</td>
                    </tr>
                  `
                    )
                    .join('')}
