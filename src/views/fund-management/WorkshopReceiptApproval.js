...
import { CBadge, CNav, CNavItem, CNavLink, CTabContent, CTabPane, CModal, CModalHeader, CModalTitle, CModalBody, CModalFooter, CButton, CFormInput } from '@coreui/react';
...
function WorkshopReceiptApproval() {
...
const fetchAllData = async () => {
  try {
    const [pendingRes, approvedRes, rejectedRes] = await Promise.all([
      axiosInstance.get('/workshop-receipts/status/pending'),
      axiosInstance.get('/workshop-receipts/status/approved'),
      axiosInstance.get('/workshop-receipts/status/rejected')
    ]);
...
const handleStatusUpdate = async (status) => {
  try {
    ...
    const response = await axiosInstance.put(
      `/workshop-receipts/${selectedVoucher._id}`,
      formData,
      {
...
const handleReject = async (voucherId) => {
  try {
    ...
    if (result.isConfirmed) {
      await axiosInstance.put(`/workshop-receipts/${voucherId}`, {
        status: 'rejected'
      });
    ...
    <div className="container-table">
      ...
      <CModal visible={showApprovalModal} onClose={() => setShowApprovalModal(false)}>
        ...
        <CModalBody>
          ...
          <div className="mb-3">
            <label htmlFor="billUpload" className="form-label">Upload Bill</label>
            <CFormInput 
              type="file" 
              id="billUpload" 
...
export default WorkshopReceiptApproval;
