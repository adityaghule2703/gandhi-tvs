...
//         try {
//           const modelIds = [...new Set(vehicles.map((v) => v.model?._id || v.model).filter((id) => id !== undefined && id !== null))];
...
//           const allAccessories = Object.values(newModelAccessories).flat();
...
//   const handleAccessoryToggle = (modelId, accessoryId) => {
//     setSelectedAccessories((prev) => ({
//       ...prev,
//       [modelId]: {
//         ...prev[modelId],
//         [accessoryId]: !prev[modelId]?.[accessoryId]
//       }
//     }));
//   };

//   const handleSelectAll = (modelId) => {
//     const modelAcc = modelAccessories[modelId] || [];
//     const allSelected = modelAcc.every((acc) => selectedAccessories[modelId]?.[acc._id]);

//     setSelectedAccessories((prev) => ({
//       ...prev,
//       [modelId]: modelAcc.reduce((acc, curr) => {
//         acc[curr._id] = !allSelected;
//         return acc;
//       }, {})
//     }));
//   };
...
//     const selectedData = Object.entries(selectedAccessories).map(([modelId, accs]) => {
//       const model = vehicles.find((v) => getModelId(v) === modelId)?.model;
//       return {
//         modelId,
//         modelName: model?.model_name || 'N/A',
//         accessories: Object.entries(accs)
//           .filter(([_, isSelected]) => isSelected)
//           .map(([accId]) => {
//             const acc = (modelAccessories[modelId] || []).find((a) => a._id === accId);
//             return acc || { _id: accId, name: 'Unknown' };
//           })
//       };
//     });

//     console.log('Selected accessories to submit:', selectedData);
//     // Here you would typically make an API call to save the selections
//     alert('Accessories selection saved! Check console for data.');
//   };
...
//         </div>
        ...
        //                         <div key={acc._id}>
        //                           <CFormCheck
        //                             id={`acc-${modelId}-${acc._id}`}
        //                             label={acc.name}
        //                             checked={!!selectedAccessories[modelId]?.[acc._id]}
        //                             onChange={() => handleAccessoryToggle(modelId, acc._id)}
        //                           />
        //                         </div>
        //                       ))}
        //                     </div>
